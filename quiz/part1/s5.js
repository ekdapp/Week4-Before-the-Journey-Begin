...
function kaliTerusRekursif(angka) {
    if (angka.length <= 1){
        ...
        angka = String(angka);
        let angkaUmum = angka.split("");
        let angkaHasil = [];
        for (let i = 0; i < angkaUmum.length; i++) {
            angkaHasil.push(parseInt(angkaUmum[i]));
        }
        let result = 1;
        for (let i = 0; i < angkaHasil.length; i++) {
            result = result * angkaHasil[i];
        }
        if (result > 9) {
